...
public class DBWorkload {
        ...
        for (String plugin : pluginList) {
            ...
            for (int i = 1; i < size + 1; i++) {
                ...
                if (activeTerminals > terminals) {
                    System.out.println("Configuration error in work " + i + ": number of active terminals" + "" + "is bigger than the total number of terminals");
                    System.exit(-1);
                }
                ...
                wrkld.addWork(time, rate, weight_strings, rateLimited, disabled, serial, timed, activeTerminals, arrival);
            }
            int j = 0;
            for (Phase p : wrkld.getAllPhases()) {
                j++;
                if (p.getWeightCount() != wrkld.getNumTxnTypes()) {
                    LOG.fatal(String.format("Configuration files is inconsistent, phase %d contains %d weights but you defined %d transaction types", j, p.getWeightCount(), wrkld.getNumTxnTypes()));
                    if (p.isSerial()) {
    ...
    private static List<String> get_weights(String plugin, SubnodeConfiguration work) {
        ...
        @SuppressWarnings(value = { "unchecked" }) List<SubnodeConfiguration> weights = work.configurationsAt("weights");
    ...
    private static Results runWorkload(List<BenchmarkModule> benchList, boolean verbose) throws QueueLimitException, IOException {
        ...
        for (BenchmarkModule bench : benchList) {
            ...
            EXEC_LOG.info(String.format("Launching the %s Benchmark with %s Phases...", bench.getBenchmarkName(), bench.getWorkloadConfiguration().getNumberOfPhases()));
