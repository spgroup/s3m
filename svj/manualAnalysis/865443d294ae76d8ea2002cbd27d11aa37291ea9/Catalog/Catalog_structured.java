...
public final class Catalog {
    ...
    public Catalog(BenchmarkModule benchmark) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("Failed to initialize %s database catalog", this.benchmark.getBenchmarkName()), ex);
        }
    }
    ...
    protected void init() throws SQLException {
        ...
        while (table_rs.next()) {
            ...
            while (col_rs.next()) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(String.format("Adding %s.%s [%s / %d]", table_name, col_name, col_typename, col_type));
                }
            ...
            while (pkey_rs.next()) {
                ...
                assert (catalog_tbl.getColumnByName(col_name) != null) : String.format("Unexpected primary key column %s.%s", table_name, col_name);
    ...
    protected Map<String, String> getOriginalTableNames() {
        ...
        assert (origTableNames.isEmpty() == false) : "Failed to extract original table names for " + this.benchmark.getBenchmarkName();
        if (LOG.isDebugEnabled()) {
