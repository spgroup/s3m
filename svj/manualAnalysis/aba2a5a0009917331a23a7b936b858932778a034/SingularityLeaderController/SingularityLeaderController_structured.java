...
public class SingularityLeaderController implements Managed, LeaderLatchListener {
    ...
    @Inject
    public SingularityLeaderController(StateManager stateManager, SingularityConfiguration configuration, SingularityAbort abort, SingularityExceptionNotifier exceptionNotifier, @Named(SingularityMainModule.HTTP_HOST_AND_PORT) HostAndPort hostAndPort, SingularityMesosScheduler scheduler, OfferCache offerCache) {
    ...
    private SingularityHostState getHostState() {
        ...
        return new SingularityHostState(master, uptime, scheduler.getState().name(), millisSinceLastOfferTimestamp, hostAndPort.getHost(), hostAndPort.getHost(), mesosMaster, scheduler.isRunning(), numCachedOffers, cachedCpus, cachedMemoryBytes);
    }
