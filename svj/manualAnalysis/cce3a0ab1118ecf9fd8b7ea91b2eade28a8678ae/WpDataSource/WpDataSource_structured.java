...
public class WpDataSource implements Closeable {
    ...
    public String translateSqlScript(String script) {
        if (dialect == SQLDialect.POSTGRES) {
            script = script.replaceAll("(?i) BIGINT AUTO_INCREMENT ", " BIGSERIAL ");
            if (script.toLowerCase().contains(" index ")) {
                script = script.replaceAll("(?i) IF NOT EXISTS ", " ");
            }
    ...
    public static class CountConnectionHook extends AbstractConnectionHook {
        ...
        public void onCheckOut(ConnectionHandle connection) {
            ...
            if ((numConnections > (numConnectionsToWarn + 1)) && (numConnections < (maxNumConnectionsToWarn + 1))) {
                log.log(Level.WARNING, "Number connections of thread more than " + String.valueOf(numConnectionsToWarn + 1) + " before check in:" + numConnections, new IllegalMonitorStateException());
            }
        }
