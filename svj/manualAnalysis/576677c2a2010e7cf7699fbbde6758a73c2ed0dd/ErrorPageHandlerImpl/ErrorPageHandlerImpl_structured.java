...
@Component(label = "ACS AEM Commons - Error Page Handler", description = "Error Page Handling module which facilitates the resolution of errors against authorable pages for discrete content trees.", immediate = false, metatype = true)
@Service
public final class ErrorPageHandlerImpl implements ErrorPageHandlerService {
    ...
    @Property(label = "Enable", description = "Enables/Disables the error handler. [Required]", boolValue = DEFAULT_ENABLED)
    ...
    @Property(label = "Error page paths", description = "List of valid inclusive content trees under which error pages may reside, along with the name of the the default error page for the content tree. This is a fallback/less powerful option to adding the ./errorPages property to CQ Page property dialogs. Example: /content/geometrixx/en:errors [Optional]", cardinality = Integer.MAX_VALUE)
    ...
    public void doHandle404(SlingHttpServletRequest request, SlingHttpServletResponse response) {
        if (componentHelper.isDisabledMode(request)) {
            ...
            if (getStatusCode(request) != SlingHttpServletResponse.SC_NOT_FOUND) {
                return;
            }
        }
    ...
    public void resetRequestAndResponse(SlingHttpServletRequest request, SlingHttpServletResponse response, int statusCode) {
        request.setAttribute(com.day.cq.widget.HtmlLibraryManager.class.getName() + ".included", new HashSet<String>());
        response.reset();
