...
public  class  AbstractRetryPolicyIntegrationTest {
  ...
  @BeforeMethod(groups = "short") public void beforeMethod() {
    scassandras = ScassandraCluster.builder().withNodes(3).build();
    scassandras.init();
    cluster = Cluster.builder().addContactPoints(scassandras.address(1).getAddress()).withPort(scassandras.getBinaryPort()).withRetryPolicy(retryPolicy).withLoadBalancingPolicy(new SortingLoadBalancingPolicy()).withPoolingOptions(new PoolingOptions().setCoreConnectionsPerHost(HostDistance.LOCAL, 1).setMaxConnectionsPerHost(HostDistance.LOCAL, 1).setHeartbeatIntervalSeconds(0)).withNettyOptions(nonQuietClusterCloseOptions).
<<<<<<< C:\Users\GUILHE~1\AppData\Local\Temp\fstmerge_var1_5695045437207623681.java
    withNettyOptions(nonQuietClusterCloseOptions).withQueryOptions(new QueryOptions().setDefaultIdempotence(true)).build()
=======
    build()
>>>>>>> C:\Users\GUILHE~1\AppData\Local\Temp\fstmerge_var2_5426708555020227341.java
    ;
  ...
  protected void simulateError(int hostNumber, Result result, Config config) {
    PrimingRequestBuilder builder = PrimingRequest.queryBuilder().withQuery("mock query").withResult(result);
    if (config != null) {
  ...
  protected void simulateNormalResponse(int hostNumber) {
    scassandras.node(hostNumber).primingClient().prime(PrimingRequest.queryBuilder().withQuery("mock query").withRows(row("result", "result1")).build());
  }
  ...
  protected void assertOnReadTimeoutWasCalled(int times) {
    Mockito.verify(retryPolicy, times(times)).onReadTimeout(any(Statement.class), any(ConsistencyLevel.class), anyInt(), anyInt(), anyBoolean(), anyInt());
  }
  ...
  protected void assertOnWriteTimeoutWasCalled(int times) {
    Mockito.verify(retryPolicy, times(times)).onWriteTimeout(any(Statement.class), any(ConsistencyLevel.class), any(WriteType.class), anyInt(), anyInt(), anyInt());
  }
  ...
  protected void assertOnUnavailableWasCalled(int times) {
    Mockito.verify(retryPolicy, times(times)).onUnavailable(any(Statement.class), any(ConsistencyLevel.class), anyInt(), anyInt(), anyInt());
  }
  ...
  protected void assertOnRequestErrorWasCalled(int times, Class<? extends DriverException> expected) {
    Mockito.verify(retryPolicy, times(times)).onRequestError(any(Statement.class), any(ConsistencyLevel.class), any(expected), anyInt());
  }
  ...
  @AfterMethod(groups = "short", alwaysRun = true) public void afterMethod() {
    if (cluster != null) {
      cluster.close();
    }
    if (scassandras != null) {
  ...
  @DataProvider public static Object[][] serverSideErrors() {
    return new Object[][] { { server_error, ServerError.class }, { overloaded, OverloadedException.class } };
  }
  ...
  @DataProvider public static Object[][] connectionErrors() {
