...
public abstract class AbstractSearch extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
    ...
    protected void buildSearchResultsDivision(Division search) throws IOException, SQLException, WingException, SearchServiceException {
        ...
        if (queryResults != null && 0 < queryResults.getTotalSearchResults()) {
            ...
            if (searchScope instanceof Community) {
                ...
                if (searchScope instanceof Collection) {
        ...
        if (queryResults != null && 0 < queryResults.getDspaceObjects().size()) {
            ...
            dspaceObjectsList = results.addList("search-results-repository", org.dspace.app.xmlui.wing.element.List.TYPE_DSO_LIST, "repository-search-results");
    ...
    protected void buildScopeList(Select scope) throws SQLException, WingException {
        ...
        if (scopeDSO == null) {
            ...
            if (scopeDSO instanceof Community) {
                ...
                if (scopeDSO instanceof Collection) {
    ...
    protected void buildSearchControls(Division div) throws WingException, SQLException {
        ...
        if (currentSort == null && searchSortConfiguration.getDefaultSort() != null) {
            currentSort = SearchUtils.getSearchService().toSortFieldIndex(searchSortConfiguration.getDefaultSort().getMetadataField(), searchSortConfiguration.getDefaultSort().getType());
        }
        String sortOrder = getParameterOrder();
        if (sortOrder == null && searchSortConfiguration.getDefaultSortOrder() != null) {
        ...
        if (searchSortConfiguration.getSortFields() != null) {
            for (DiscoverySortFieldConfiguration sortFieldConfiguration : searchSortConfiguration.getSortFields()) {
                ...
                boolean selectedAsc = sortField.equals(currentSort) && SORT_ORDER.asc.name().equals(sortOrder);
                boolean selectedDesc = sortField.equals(currentSort) && SORT_ORDER.desc.name().equals(sortOrder);
    ...
    protected void logSearch() {
        ...
        try {
            ...
            if (dsoScope instanceof Collection) {
                ...
                if (dsoScope instanceof Community) {
                    logInfo = "community_id=" + dsoScope.getID() + ",";
                }
            }
        } catch (SQLException sqle) {
        }
        log.info(LogManager.getHeader(context, "search", logInfo + "query=\"" + (queryArgs == null ? "" : queryArgs.getQuery()) + "\",results=(" + countCommunities + "," + countCollections + "," + countItems + ")"));
    }
}
