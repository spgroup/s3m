...
public class TwigFormatter implements IStructuredFormatter {
                        ...
                        if (startFormat <= start && endFormat >= end) {
                            if (phpToken.getType().equals(TwigRegionTypes.PHP_CURLY_OPEN)) {
                                if (j < twigTokens.length - 1 && j > 0) {
                                    if (twigTokens[j - 1].getType().equals(TwigRegionTypes.TWIG_DELIMITER) && document.getLineOfOffset(startOffset + phpToken.getStart()) == document.getLineOfOffset(startOffset + twigTokens[j + 1].getStart())) {
                                        document.replace(startOffset + phpToken.getEnd(), 0, newline);
                                        endLineIndex++;
                                    }
                                }
                            } else {
                                if (phpToken.getType().equals(TwigRegionTypes.PHP_CURLY_CLOSE)) {
                                    if (j > 0 && (twigTokens[j - 1].getType().equals(TwigRegionTypes.PHP_SEMICOLON) || twigTokens[j - 1].getType().equals(TwigRegionTypes.PHP_CURLY_CLOSE) || twigTokens[j - 1].getType().equals(TwigRegionTypes.TWIG_COMMENT_CLOSE))) {
                                        if (document.getLineOfOffset(startOffset + phpToken.getStart()) == document.getLineOfOffset(startOffset + twigTokens[j - 1].getStart())) {
    ...
    private void formatLine(IStructuredDocument document, int lineNumber) {
        ...
        try {
            ...
            if (firstTokenInLine instanceof ITwigScriptRegion) {
                ...
                if (firstTokenInLine != null && firstTokenInLine.getStart() + sdRegion.getStartOffset() < orginalLineStart && firstTokenInLine.getType() == TwigRegionTypes.WHITESPACE) {
                    firstTokenInLine = scriptRegion.getTwigToken(formattedLineStart - regionStart + firstTokenInLine.getLength());
                }
            }
            ...
            if (isTwigCommentRegion(firstTokenType)) {
                ...
                if (firstTokenType == TwigRegionTypes.TWIG_CLOSETAG) {
    ...
    private boolean isTwigCommentRegion(String tokenType) {
        return (tokenType == TwigRegionTypes.TWIG_COMMENT || tokenType == TwigRegionTypes.TWIG_COMMENT_TEXT || tokenType == TwigRegionTypes.TWIG_COMMENT_CLOSE);
    }
    ...
    private IRegion getFormattedLineInformation(IRegion lineInfo, String lineText) {
        ...
        while (keepSearching) {
            final boolean leftIsWhiteSpace = chars[leftNonWhitespaceChar] == CHAR_SPACE || chars[leftNonWhitespaceChar] == CHAR_TAB;
            final boolean rightIsWhiteSpace = chars[rightNonWhitespaceChar] == CHAR_SPACE || chars[rightNonWhitespaceChar] == CHAR_TAB;
            if (leftIsWhiteSpace) {
    ...
    private boolean shouldReformat(IStructuredDocument document, IRegion lineInfo) {
        ...
        return ((checkedLineBeginState == TwigPartitionTypes.TWIG_DEFAULT) || (checkedLineBeginState == TwigPartitionTypes.TWIG_COMMENT_TEXT)) || (checkedLineBeginState == TwigPartitionTypes.TWIG_COMMENT_TEXT);
    }
