...
public final class SegmentRunner implements RepairStatusHandler {
    ...
    public static void triggerRepair(IStorage storage, long segmentId, Collection<String> potentialCoordinators, long timeoutMillis, JmxConnectionFactory jmxConnectionFactory) {
        new SegmentRunner(storage, segmentId).runRepair(potentialCoordinators, jmxConnectionFactory, timeoutMillis);
    }

    public static void postpone(IStorage storage, RepairSegment segment) {
        LOG.warn("Postponing segment {}", segment.getId());
        storage.updateRepairSegment(segment.with().state(RepairSegment.State.NOT_STARTED).coordinatorHost(null).repairCommandId(null).startTime(null).failCount(segment.getFailCount() + 1).build(segment.getId()));
    }
    ...
    private void runRepair(Collection<String> potentialCoordinators, JmxConnectionFactory jmxConnectionFactory, long timeoutMillis) {
        final RepairSegment segment = storage.getRepairSegment(segmentId).get();
        try (JmxProxy coordinator = jmxConnectionFactory.connectAny(Optional.<RepairStatusHandler>of(this), potentialCoordinators)) {
            ...
            synchronized (condition) {
                commandId = coordinator.triggerRepair(segment.getStartToken(), segment.getEndToken(), keyspace, repairUnit.getColumnFamilies());
                LOG.debug("Triggered repair with command id {}", commandId);
                storage.updateRepairSegment(segment.with().state(RepairSegment.State.RUNNING).coordinatorHost(coordinator.getHost()).repairCommandId(commandId).build(segmentId));
                ...
                try {
                    ...
                    LOG.info("Repair command {} on segment {} exited with state {}", commandId, segmentId, resultingSegment.getState());
    ...
    boolean canRepair(RepairSegment segment, String keyspace, JmxProxy coordinator, JmxConnectionFactory factory) throws ReaperException {
        Collection<JmxProxy> allHosts = factory.connectAll(coordinator.tokenRangeToEndpoint(keyspace, segment.getTokenRange()));
        for (JmxProxy host : allHosts) {
            if (host.getPendingCompactions() > MAX_PENDING_COMPACTIONS) {
                LOG.warn("SegmentRunner declined to repair segment {} because of too many pending " + "compactions (> {}) on host \"{}\"", segmentId, MAX_PENDING_COMPACTIONS, host.getHost());
                return false;
            }
        }
        return true;
   ...
   */
    ...
    public void handle(int repairNumber, ActiveRepairService.Status status, String message) {
        synchronized (condition) {
            LOG.debug("handle called for repairCommandId {}, outcome {} and message: {}", repairNumber, status, message);
            ...
            switch(status) {
                case STARTED:
                    ...
                    storage.updateRepairSegment(currentSegment.with().startTime(now).build(segmentId));
                    break;
                case SESSION_FAILED:
                ...
                case SESSION_SUCCESS:
                    break;
                case FINISHED:
                    storage.updateRepairSegment(currentSegment.with().state(RepairSegment.State.DONE).endTime(DateTime.now()).build(segmentId));
                    condition.signalAll();
                    break;
            }
        }
