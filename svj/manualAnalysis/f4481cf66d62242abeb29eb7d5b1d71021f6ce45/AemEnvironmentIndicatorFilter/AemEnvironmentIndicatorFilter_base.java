...
public class AemEnvironmentIndicatorFilter implements Filter {
    ...
    @Property(label = "CSS Override", description = "Accepts any valid CSS to style the AEM indicator div. All CSS rules must only be " + "scoped to #" + DIV_ID + " { .. }", value = "")
    public static final String PROP_CSS_OVERRIDE = "css-override";

    /* Property: Inner HTML */
    private String innerHTML = "";

    @Property(label = "Inner HTML", description = "Any additional HTML required; Will be injected into a div with" + " id=\'" + DIV_ID + "\'", value = "")
    ...
    @Property(label = "Browser Title", description = "A prefix to add to the browser tab/window title; <THIS VALUE> | <ORIGINAL DOC TITLE>", value = DEFAULT_TITLE_PREFIX)
        ...
        if (doInclude && contents != null && StringUtils.contains(response.getContentType(), "html")) {
            ...
            if (bodyIndex != -1) {
                ...
                if (StringUtils.isNotBlank(titlePrefix)) {
                    printWriter.write("<script>document.title = \'" + titlePrefix + " | \' + document.title;</script>");
                }
    ...
    private boolean accepts(final HttpServletRequest request) {
        if (StringUtils.isBlank(css) && StringUtils.isBlank(titlePrefix)) {
            ...
            if (!StringUtils.equalsIgnoreCase("get", request.getMethod())) {
                ...
                if (StringUtils.startsWithAny(request.getRequestURI(), REJECT_PATH_PREFIXES)) {
                    return false;
                } else {
                    if (StringUtils.equals(request.getHeader("X-Requested-With"), "XMLHttpRequest")) {
                        return false;
                    } else {
                        if (StringUtils.endsWith(request.getHeader("Referer"), "/editor.html" + request.getRequestURI())) {
                            return false;
                        } else {
                            if (StringUtils.endsWith(request.getHeader("Referer"), "/cf")) {
                                return false;
                            }
                        }
                    }
    ...
    private String createCSS(final String providedColor) {
        return "#" + DIV_ID + " { " + "background-color:" + providedColor + BASE_DEFAULT_STYLE + " }";
    }
    ...
    private WCMMode extractFromRequest(HttpServletRequest request) {
        return (WCMMode) request.getAttribute(WCMMode.REQUEST_ATTRIBUTE_NAME);
    }
}
