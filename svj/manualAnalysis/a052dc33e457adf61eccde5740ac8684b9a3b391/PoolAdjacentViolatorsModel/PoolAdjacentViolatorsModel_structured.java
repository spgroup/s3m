...
public class PoolAdjacentViolatorsModel implements SingleVariableRealValuedFunction {
    ...
    public Double predict(Double input) {
        ...
        if (!interpolateThroughOrigin && floor == null && calibrationSet.higher(calibrationSet.first()) != null) {
            ...
            try {
                double slopeOffEnd = (upperYCoord - calibrationSet.first().output) / (upperXCoord - calibrationSet.first().input);
        ...
        if (ceiling == null && extropolateOffUpperEnd) {
            ...
            try {
                double slopeOffEnd = (calibrationSet.last().output - lowerYCoord) / (calibrationSet.last().input - lowerXcoord);
    ...
    public static final class Observation implements Comparable<Observation>, Serializable {
        ...
        public Observation mergeWith(final Observation other) {
            ...
            return new Observation((input * weight + other.input * other.weight) / (weight + other.weight), (output * weight + other.output * other.weight) / (weight + other.weight), weight + other.weight);
        }
