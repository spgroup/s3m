...
public class SingularityNewTaskChecker {
    ...
    @Inject
    public SingularityNewTaskChecker(@Named(value = SingularityMainModule.NEW_TASK_THREADPOOL_NAME) ScheduledExecutorService executorService, RequestManager requestManager, SingularityConfiguration configuration, LoadBalancerClient lbClient, TaskManager taskManager, SingularityExceptionNotifier exceptionNotifier, SingularityAbort abort, SingularityDeployHealthHelper deployHealthHelper, DisasterManager disasterManager) {
    ...
    public void runNewTaskCheckImmediately(SingularityTask task, SingularityHealthchecker healthchecker) {
        ...
        if (cancelState == CancelState.NOT_CANCELED) {
            ...
            if (cancelState == CancelState.NOT_PRESENT) {
    ...
    private boolean unknownNotRemoving(SingularityLoadBalancerUpdate update) {
        return update.getLoadBalancerState() == BaragonRequestState.UNKNOWN && update.getLoadBalancerRequestId().getRequestType() != LoadBalancerRequestType.REMOVE;
    }
}
