...
public class AgentlessInstaller {
    ...
    private static final int DEFAULT_ROUTE_RESOLUTION_TIMEOUT = 2 * 60 * 1000;
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(AgentlessInstaller.class.getName());
    ...
    private static final String[] POWERSHELL_INSTALLED_COMMAND = new String[] { "powershell.exe", "-inputformat", "none", "-?" };
    ...
    private static InetAddress waitForRoute(final String ip, final long endTime) throws InstallerException, InterruptedException {
	 ...
	 * @throws InterruptedException
	 */
    private void copyFiles(final InstallationDetails details, final String host, final Set<String> excludedFiles, final long timeout, final TimeUnit unit) throws IOException, TimeoutException, URISyntaxException, InterruptedException, InstallerException {
        if (timeout < 0) {
    ...
    private FileObject resolveTargetDirectory(final FileSystemOptions opts, final String target, final FileSystemManager mng) throws FileSystemException {
    ...
    private FileSystemManager createRemoteSSHFileSystem(final String keyFile, final FileSystemOptions opts, final InstallationDetails details) throws FileSystemException, FileNotFoundException {
	 ...
	 */
    public void uninstallMachine(final InstallationDetails details, final long timeout, final TimeUnit unit) throws InstallerException, TimeoutException, InterruptedException {
        deleteRemoteFiles(details, timeout, unit);
    }

    private void deleteRemoteFiles(final InstallationDetails details, final long timeout, final TimeUnit unit) throws TimeoutException, InstallerException, InterruptedException {
	 ...
	 * @throws InstallerException .
	 */
    public void installOnMachineWithIP(final InstallationDetails details, final long timeout, final TimeUnit unit) throws TimeoutException, InterruptedException, InstallerException {
        final long end = System.currentTimeMillis() + unit.toMillis(timeout);
    ...
    private void remoteExecuteAgentOnServer(final InstallationDetails details, final long end, final String targetHost) throws InstallerException, TimeoutException, InterruptedException {
    ...
    private void uploadFilesToServer(final InstallationDetails details, final long end, final String targetHost) throws TimeoutException, InstallerException, InterruptedException {
        try {
    ...
    private List<String> getPowershellCommandLine(final String target, final String username, final String password, final String command, final String localDir) throws FileNotFoundException {
        final File clientScriptFile = new File(localDir, POWERSHELL_CLIENT_SCRIPT);
        if (!clientScriptFile.exists()) {
            throw new FileNotFoundException("Could not find expected powershell client script in local directory. Was expecting file: " + clientScriptFile.getAbsolutePath());
        }
    ...
    private void powershellCommand(final String targetHost, final String command, final String username, final String password, final String keyFile, final long millisUntil, final TimeUnit milliseconds, final String localDir) throws InstallerException, InterruptedException, TimeoutException {
    ...
    private String invokeRemotePowershellCommand(final String targetHost, final String command, final String username, final String password, final String localDir) throws InstallerException, InterruptedException, FileNotFoundException {
    ...
    private void checkPowershellInstalled() throws IOException, InterruptedException, InstallerException {
        if (powerShellInstalled != null) {
            if (powerShellInstalled.booleanValue()) {
                ...
                throw new InstallerException("powershell.exe is not on installed, or is not available on the system path. " + "Powershell is required on both client and server for Cloudify to work on Windows. ");
            }
        }
        ...
        if (retval != 0) {
            throw new InstallerException("powershell.exe is not on installed, or is not available on the system path. " + "Powershell is required on both client and server for Cloudify to work on Windows. " + "Execution result: " + output);
        }
