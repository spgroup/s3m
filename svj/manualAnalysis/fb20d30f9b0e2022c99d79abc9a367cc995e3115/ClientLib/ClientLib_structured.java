...
public class ClientLib implements ClientAPI, DbgAPI, ConfigAPI {
    ...
    public static CorfuConfiguration pullConfigUtil() throws CorfuException {
    ...
    public static CorfuConfiguration pullConfig(String master) throws CorfuException {
        DefaultHttpClient httpclient = new DefaultHttpClient();
        try {
            HttpGet httpget = new HttpGet(master);
            System.out.println("Executing request: " + httpget.getRequestLine());
    ...
    public void writeExtnt(long offset, List<ByteBuffer> ctnt) throws CorfuException {
        ...
        try {
            for (Endpoint ep : replicas) {
                ...
                log.info("err={} write{} replica={} size={} marktype={}", er, offset, ep, ctnt.size(), ExtntMarkType.EX_FILLED);
        ...
        if (er == null || er.equals(ErrorCode.ERR_STALEEPOCH)) {
            ...
            pullConfig();
            if (Util.compareIncarnations(CM.getIncarnation(), curepoch) > 0) {
                writeExtnt(offset, ctnt);
            } else {
                throw new ConfigCorfuException("append() failed: incarnation " + curepoch + " not responding, err=" + er);
            }
        } else {
    ...
    public ExtntWrap readExtnt(long offset) throws CorfuException {
        ...
        for (ListIterator<Endpoint> it = el.group.listIterator(el.group.size()); it.hasPrevious() && ep == null; ep = it.previous()) {
            ;
        }
        sunit = Endpoint.getSUnitOf(ep);
        try {
    ...
    public long querytail() throws CorfuException {
        ...
        if (r < 0) {
            throw new InternalCorfuException("querytail() call returned negative value, shouldn\'t happen");
        }
        return r;
    }
    ...
    public void killUnit(int gind, int rind) throws CorfuException {
        ...
        if (ep == null) {
            throw new ConfigCorfuException("nothing to kill");
        }
        try {
