...
public class ITestDtUpdateOperation extends ITestBase {
    ...
    private CrdtCounter fetchCounter(BinaryValue type, BinaryValue bucket, BinaryValue key) throws ExecutionException, InterruptedException {
    ...
    private CrdtSet fetchSet(BinaryValue type, BinaryValue bucket, BinaryValue key) throws ExecutionException, InterruptedException {
        DtFetchOperation fetch = new DtFetchOperation.Builder(bucket, key).withBucketType(type).build();
        cluster.execute(fetch);
        DtFetchOperation.Response response = fetch.get();
        CrdtElement element = response.getCrdtElement();
    ...
    private CrdtMap fetchMap(BinaryValue type, BinaryValue bucket, BinaryValue key) throws ExecutionException, InterruptedException {
        DtFetchOperation fetch = new DtFetchOperation.Builder(bucket, key).withBucketType(type).build();
        cluster.execute(fetch);
        DtFetchOperation.Response response = fetch.get();
        CrdtElement element = response.getCrdtElement();
    ...
    public void testCrdtCounter() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < iterations; ++i) {
            DtUpdateOperation update = new DtUpdateOperation.Builder(bucketName, counterBucketType).withKey(key).withOp(new CounterOp(1)).build();
    ...
    public void testCrdtSet() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            DtUpdateOperation update = new DtUpdateOperation.Builder(bucketName, setBucketType).withOp(new SetOp().add(wrapped)).withKey(key).build();
            cluster.execute(update);
            update.get();
        }
        ...
        for (BinaryValue setElement : testValues) {
            DtUpdateOperation update = new DtUpdateOperation.Builder(bucketName, setBucketType).withOp(new SetOp().remove(setElement)).withKey(key).build();
            cluster.execute(update);
            update.get();
        }
        set = fetchSet(setBucketType, bucketName, key);
    ...
    public void testCrdtSetInterleved() throws ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < iterations; ++i) {
            ...
            BinaryValue wrapped = BinaryValue.create(buff.array());
            testValues.add(wrapped);
            DtUpdateOperation add = new DtUpdateOperation.Builder(bucketName, setBucketType).withOp(new SetOp().add(wrapped)).withKey(key).build();
            cluster.execute(add);
            add.get();
            DtUpdateOperation delete = new DtUpdateOperation.Builder(bucketName, setBucketType).withOp(new SetOp().remove(wrapped)).withKey(key).build();
    ...
    public void testCrdtMap() throws ExecutionException, InterruptedException {
        ...
        BinaryValue mapKey = BinaryValue.create("asSet");
        DtUpdateOperation add = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().add(mapKey, MapOp.FieldType.SET)).withKey(key).build();
        cluster.execute(add);
        add.get();
        DtUpdateOperation update = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().update(mapKey, new SetOp().add(setValue))).withKey(key).build();
        cluster.execute(update);
        update.get();
        ...
        add = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().add(mapKey, MapOp.FieldType.COUNTER)).withKey(key).build();
        cluster.execute(add);
        add.get();
        update = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().update(mapKey, new CounterOp(1))).withKey(key).build();
        cluster.execute(update);
        update.get();
        map = fetchMap(mapBucketType, bucketName, key);
        ...
        DtUpdateOperation addSet = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().add(mapKey, MapOp.FieldType.FLAG)).withKey(key).build();
        cluster.execute(addSet);
        addSet.get();
        add = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().update(mapKey, new FlagOp(true))).withKey(key).build();
        cluster.execute(add);
        add.get();
        map = fetchMap(mapBucketType, bucketName, key);
        ...
        addSet = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().add(mapKey, MapOp.FieldType.REGISTER)).withKey(key).build();
        cluster.execute(addSet);
        addSet.get();
        add = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().update(mapKey, new RegisterOp(mapKey))).withKey(key).build();
        cluster.execute(add);
        add.get();
        map = fetchMap(mapBucketType, bucketName, key);
        ...
        mapKey = BinaryValue.create("asMap");
        addSet = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().add(mapKey, MapOp.FieldType.MAP)).withKey(key).build();
        cluster.execute(addSet);
        addSet.get();
        add = new DtUpdateOperation.Builder(bucketName, mapBucketType).withOp(new MapOp().update(mapKey, new MapOp().add(mapKey, MapOp.FieldType.FLAG))).withKey(key).build();
        cluster.execute(add);
        add.get();
        map = fetchMap(mapBucketType, bucketName, key);
