...
public class UpgradeTest {
    ...
    private void upgrade(Version sqVersion) {
        ...
        assertThat(serverMigrationResponse.getStatus()).describedAs("Migration status of version " + sqVersion + " should be MIGRATION_SUCCEEDED").isEqualTo(ServerMigrationResponse.Status.MIGRATION_SUCCEEDED);
    ...
    private void checkSystemStatus(Version sqVersion, ServerStatusResponse.Status serverStatus) {
        ...
        assertThat(serverStatusResponse.getStatus()).describedAs("Server status of version " + sqVersion + " should be " + serverStatus).isEqualTo(serverStatus);
    }
    ...
    private void scanProject() {
        MavenBuild build = MavenBuild.create(new File("projects/struts-1.3.9-diet/pom.xml")).setCleanPackageSonarGoals().setProperty("sonar.exclusions", "**/pom.xml").setProperty("sonar.dynamicAnalysis", "false").setProperty("sonar.scm.disabled", "true").setProperty("sonar.cpd.cross_project", "true");
        orchestrator.executeBuild(build);
    }

    private int countFiles(String key) {
        if (orchestrator.getConfiguration().getSonarVersion().isGreaterThanOrEquals("5.4")) {
            Measure measure = newWsClient(orchestrator).measures().component(new ComponentWsRequest().setComponentKey(key).setMetricKeys(Collections.singletonList("files"))).getComponent().getMeasures(0);
    ...
    private static WsClient newWsClient(Orchestrator orchestrator) {
        ...
        return WsClientFactories.getDefault().newClient(HttpConnector.newBuilder().url(server.getUrl()).build());
    }
