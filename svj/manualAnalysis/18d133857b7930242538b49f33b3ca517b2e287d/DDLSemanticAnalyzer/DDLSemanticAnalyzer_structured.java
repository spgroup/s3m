...
public class DDLSemanticAnalyzer extends BaseSemanticAnalyzer {
            ...
            case HiveParser.TOK_ALTERTABLE:
                {
                    ...
                    if (ast.getType() == HiveParser.TOK_ALTERTABLE_RENAME) {
                        ...
                        if (ast.getType() == HiveParser.TOK_ALTERTABLE_TOUCH) {
                            analyzeAlterTableTouch(qualified, ast);
                        } else {
                            if (ast.getType() == HiveParser.TOK_ALTERTABLE_ARCHIVE) {
                                analyzeAlterTableArchive(qualified, ast, false);
                            } else {
                                if (ast.getType() == HiveParser.TOK_ALTERTABLE_UNARCHIVE) {
                                    analyzeAlterTableArchive(qualified, ast, true);
                                } else {
                                    if (ast.getType() == HiveParser.TOK_ALTERTABLE_ADDCOLS) {
                                        analyzeAlterTableModifyCols(qualified, ast, partSpec, AlterTableTypes.ADDCOLS);
                                    } else {
                                        if (ast.getType() == HiveParser.TOK_ALTERTABLE_REPLACECOLS) {
                                            analyzeAlterTableModifyCols(qualified, ast, partSpec, AlterTableTypes.REPLACECOLS);
                                        } else {
                                            if (ast.getType() == HiveParser.TOK_ALTERTABLE_RENAMECOL) {
                                                analyzeAlterTableRenameCol(qualified, ast, partSpec);
                                            } else {
                                                if (ast.getType() == HiveParser.TOK_ALTERTABLE_ADDPARTS) {
                                                    analyzeAlterTableAddParts(qualified, ast, false);
                                                } else {
                                                    if (ast.getType() == HiveParser.TOK_ALTERTABLE_DROPPARTS) {
                                                        analyzeAlterTableDropParts(qualified, ast, false);
                                                    } else {
                                                        if (ast.getType() == HiveParser.TOK_ALTERTABLE_PARTCOLTYPE) {
                                                            analyzeAlterTablePartColType(qualified, ast);
                                                        } else {
                                                            if (ast.getType() == HiveParser.TOK_ALTERTABLE_PROPERTIES) {
                                                                analyzeAlterTableProps(qualified, null, ast, false, false);
                                                            } else {
                                                                if (ast.getType() == HiveParser.TOK_ALTERTABLE_DROPPROPERTIES) {
                                                                    analyzeAlterTableProps(qualified, null, ast, false, true);
                                                                } else {
                                                                    if (ast.getType() == HiveParser.TOK_ALTERTABLE_UPDATESTATS) {
                                                                        analyzeAlterTableProps(qualified, partSpec, ast, false, false);
                                                                    } else {
                                                                        if (ast.getType() == HiveParser.TOK_ALTERTABLE_SKEWED) {
                                                                            analyzeAltertableSkewedby(qualified, ast);
                                                                        } else {
                                                                            if (ast.getType() == HiveParser.TOK_ALTERTABLE_EXCHANGEPARTITION) {
                                                                                ...
                                                                                if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_FILEFORMAT) {
                                                                                    ...
                                                                                    if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_LOCATION) {
                                                                                        analyzeAlterTableLocation(ast, tableName, partSpec);
                                                                                    } else {
                                                                                        if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_MERGEFILES) {
                                                                                            analyzeAlterTablePartMergeFiles(ast, tableName, partSpec);
                                                                                        } else {
                                                                                            if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_SERIALIZER) {
                                                                                                analyzeAlterTableSerde(ast, tableName, partSpec);
                                                                                            } else {
                                                                                                if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_SERDEPROPERTIES) {
                                                                                                    analyzeAlterTableSerdeProps(ast, tableName, partSpec);
                                                                                                } else {
                                                                                                    if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_RENAMEPART) {
                                                                                                        analyzeAlterTableRenamePart(ast, tableName, partSpec);
                                                                                                    } else {
                                                                                                        if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_SKEWED_LOCATION) {
                                                                                                            analyzeAlterTableSkewedLocation(ast, tableName, partSpec);
                                                                                                        } else {
                                                                                                            if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_BUCKETS) {
                                                                                                                analyzeAlterTableBucketNum(ast, tableName, partSpec);
                                                                                                            } else {
                                                                                                                if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_CLUSTER_SORT) {
                                                                                                                    analyzeAlterTableClusterSort(ast, tableName, partSpec);
                                                                                                                } else {
                                                                                                                    if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_COMPACT) {
                                                                                                                        analyzeAlterTableCompact(ast, tableName, partSpec);
                                                                                                                    } else {
                                                                                                                        if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_UPDATECOLSTATS) {
                                                                                                                            analyzeAlterTableUpdateStats(ast, tableName, partSpec);
                                                                                                                        } else {
                                                                                                                            if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_DROPCONSTRAINT) {
                                                                                                                                analyzeAlterTableDropConstraint(ast, tableName);
                                                                                                                            } else {
                                                                                                                                if (ast.getToken().getType() == HiveParser.TOK_ALTERTABLE_ADDCONSTRAINT) {
            ...
            case HiveParser.TOK_ALTERVIEW:
                {
                    ...
                    if (ast.getType() == HiveParser.TOK_ALTERVIEW_PROPERTIES) {
                        ...
                        if (ast.getType() == HiveParser.TOK_ALTERVIEW_DROPPROPERTIES) {
                            analyzeAlterTableProps(qualified, null, ast, true, true);
                        } else {
                            if (ast.getType() == HiveParser.TOK_ALTERVIEW_ADDPARTS) {
                                analyzeAlterTableAddParts(qualified, ast, true);
                            } else {
                                if (ast.getType() == HiveParser.TOK_ALTERVIEW_DROPPARTS) {
                                    analyzeAlterTableDropParts(qualified, ast, true);
                                } else {
                                    if (ast.getType() == HiveParser.TOK_ALTERVIEW_RENAME) {
    ...
    private void analyzeSetShowRole(ASTNode ast) throws SemanticException {
        switch(ast.getChildCount()) {
            case 0:
                ctx.setResFile(ctx.getLocalTmpPath());
                rootTasks.add(hiveAuthorizationTaskFactory.createShowCurrentRoleTask(getInputs(), getOutputs(), ctx.getResFile()));
                setFetchTask(createFetchTask(RoleDDLDesc.getRoleNameSchema()));
                break;
            case 1:
                rootTasks.add(hiveAuthorizationTaskFactory.createSetRoleTask(BaseSemanticAnalyzer.unescapeIdentifier(ast.getChild(0).getText()), getInputs(), getOutputs()));
                break;
            default:
    ...
    private void analyzeShowGrant(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createShowGrantTask(ast, ctx.getResFile(), getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
    ...
    private void analyzeGrant(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createGrantTask(ast, getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
        }
    }
    ...
    private void analyzeRevoke(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createRevokeTask(ast, getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
        }
    }
    ...
    private void analyzeCreateRole(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createCreateRoleTask(ast, getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
        }
    }
    ...
    private void analyzeDropRole(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createDropRoleTask(ast, getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
        }
    }
    ...
    private void analyzeShowRoleGrant(ASTNode ast) throws SemanticException {
        Task<? extends Serializable> task = hiveAuthorizationTaskFactory.createShowRoleGrantTask(ast, ctx.getResFile(), getInputs(), getOutputs());
        if (task != null) {
            rootTasks.add(task);
    ...
    private void analyzeShowRolePrincipals(ASTNode ast) throws SemanticException {
        Task<DDLWork> roleDDLTask = (Task<DDLWork>) hiveAuthorizationTaskFactory.createShowRolePrincipalsTask(ast, ctx.getResFile(), getInputs(), getOutputs());
        if (roleDDLTask != null) {
    ...
    private void analyzeShowRoles(ASTNode ast) throws SemanticException {
        @SuppressWarnings(value = { "unchecked" }) Task<DDLWork> roleDDLTask = (Task<DDLWork>) hiveAuthorizationTaskFactory.createShowRolesTask(ast, ctx.getResFile(), getInputs(), getOutputs());
        if (roleDDLTask != null) {
            rootTasks.add(roleDDLTask);
            setFetchTask(createFetchTask(RoleDDLDesc.getRoleNameSchema()));
        }
   ...
   */
    ...
    private void analyzeTruncateTable(ASTNode ast) throws SemanticException {
        ...
        if (ast.getChildCount() == 2) {
            try {
                ...
                LoadTableDesc ltd = new LoadTableDesc(queryTmpdir, tblDesc, partSpec == null ? new HashMap<String, String>() : partSpec, null);
                ltd.setLbCtx(lbCtx);
                @SuppressWarnings(value = { "unchecked" }) Task<MoveWork> moveTsk = TaskFactory.get(new MoveWork(null, null, ltd, null, false), conf);
    ...
    private void validateAlterTableType(Table tbl, AlterTableTypes op, boolean expectView) throws SemanticException {
        if (tbl.isView()) {
            if (!expectView) {
    ...
    private void analyzeAlterTableProps(String[] qualified, HashMap<String, String> partSpec, ASTNode ast, boolean expectView, boolean isUnset) throws SemanticException {
        ...
        boolean isPotentialMmSwitch = mapProp.containsKey(hive_metastoreConstants.TABLE_IS_TRANSACTIONAL) || mapProp.containsKey(hive_metastoreConstants.TABLE_TRANSACTIONAL_PROPERTIES);
    ...
    private void addInputsOutputsAlterTable(String tableName, Map<String, String> partSpec, AlterTableTypes op) throws SemanticException {
    ...
    private void addInputsOutputsAlterTable(String tableName, Map<String, String> partSpec, AlterTableDesc desc, AlterTableTypes op, boolean doForceExclusive) throws SemanticException {
        ...
        WriteEntity.WriteType writeType = doForceExclusive ? WriteType.DDL_EXCLUSIVE : WriteEntity.determineAlterTableWriteType(op);
        if (!alterPartitions) {
            inputs.add(new ReadEntity(tab));
    ...
    private void analyzeAlterTablePartMergeFiles(ASTNode ast, String tableName, HashMap<String, String> partSpec) throws SemanticException {
        ...
        try {
            ...
            LoadTableDesc ltd = new LoadTableDesc(queryTmpdir, tblDesc, partSpec == null ? new HashMap<>() : partSpec, null);
            ltd.setLbCtx(lbCtx);
     ...
     */
        public static String getFullyQualifiedName(ASTNode ast) {
            if (ast.getChildCount() == 0) {
                ...
                if (ast.getChildCount() == 2) {
                    return getFullyQualifiedName((ASTNode) ast.getChild(0)) + "." + getFullyQualifiedName((ASTNode) ast.getChild(1));
                } else {
                    if (ast.getChildCount() == 3) {
                        return getFullyQualifiedName((ASTNode) ast.getChild(0)) + "." + getFullyQualifiedName((ASTNode) ast.getChild(1)) + "." + getFullyQualifiedName((ASTNode) ast.getChild(2));
                    } else {
                        return null;
                    }
                }
        ...
        if (ast.getChildCount() >= 1) {
            for (int i = 0; i < ast.getChildCount(); i++) {
                ...
                if (child.getType() == HiveParser.TOK_TABTYPE) {
                    ...
                    if (child.getType() == HiveParser.KW_EXTENDED) {
                        isExtended = true;
                    }
                }
            }
   ...
   */
    ...
    private void analyzeShowConf(ASTNode ast) throws SemanticException {
        ...
        rootTasks.add(TaskFactory.get(new DDLWork(getInputs(), getOutputs(), showConfDesc), conf));
   ...
   */
    ...
    private void analyzeAlterTableRenameCol(String[] qualified, ASTNode ast, HashMap<String, String> partSpec) throws SemanticException {
        ...
        AlterTableDesc alterTblDesc;
        if (primaryKeys == null && foreignKeys == null && uniqueConstraints == null && notNullConstraints == null) {
            ...
            alterTblDesc = new AlterTableDesc(tblName, partSpec, unescapeIdentifier(oldColName), unescapeIdentifier(newColName), newType, newComment, first, flagCol, isCascade, primaryKeys, foreignKeys, uniqueConstraints, notNullConstraints);
        }
            ...
            for (int i = 0; i < partSpecTree.getChildCount(); ++i) {
                ...
                if (!isDefaultPartitionName) {
                    ...
                    if (FunctionRegistry.isEq(originalOp)) {
                        ...
                        if (FunctionRegistry.isNeq(originalOp)) {
                            fnName = "isnotnull";
                        } else {
