...
public class SardineImpl implements Sardine {
    ...
    public <T extends java.lang.Object> T put(String url, HttpEntity entity, Map<String, String> headers, ResponseHandler<T> handler) throws IOException {
    ...
    protected HttpClientBuilder configure(ProxySelector selector, CredentialsProvider credentials) {
        ...
        return HttpClients.custom().setUserAgent("Sardine/" + version).setDefaultCredentialsProvider(credentials).setRedirectStrategy(this.createDefaultRedirectStrategy()).setDefaultRequestConfig(RequestConfig.custom().setExpectContinueEnabled(// Only selectively enable this for PUT but not all entity enclosing methods
        false).build()).setConnectionManager(cm).setRoutePlanner(this.createDefaultRoutePlanner(this.createDefaultSchemePortResolver(), selector));
    }
