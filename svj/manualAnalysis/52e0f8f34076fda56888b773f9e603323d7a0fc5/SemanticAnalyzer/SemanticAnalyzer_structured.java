...
public class SemanticAnalyzer extends BaseSemanticAnalyzer {
   ...
   */
    ...
    private final Set<Integer> ignoredTokens = Sets.newHashSet(HiveParser.TOK_GROUPBY, HiveParser.TOK_ORDERBY, HiveParser.TOK_WINDOWSPEC, HiveParser.TOK_CLUSTERBY, HiveParser.TOK_DISTRIBUTEBY, HiveParser.TOK_SORTBY);
   ...
   */
    private void doPhase1GetAllAggregations(ASTNode expressionTree, HashMap<String, ASTNode> aggregations, List<ASTNode> wdwFns, ASTNode wndParent) throws SemanticException {
        ...
        for (int i = 0; i < expressionTree.getChildCount(); i++) {
            doPhase1GetAllAggregations((ASTNode) expressionTree.getChild(i), aggregations, wdwFns, wndParent);
        }
    }
    ...
    int[] findTabRefIdxs(ASTNode tabref) {
        ...
        for (int index = 1; index < tabref.getChildCount(); index++) {
            ...
            if (ct.getToken().getType() == HiveParser.TOK_TABLEBUCKETSAMPLE) {
                ...
                if (ct.getToken().getType() == HiveParser.TOK_TABLESPLITSAMPLE) {
                    ssampleIndex = index;
                } else {
                    if (ct.getToken().getType() == HiveParser.TOK_TABLEPROPERTIES) {
                        propsIndex = index;
                    } else {
                        aliasIndex = index;
                    }
   ...
   */
    private String processTable(QB qb, ASTNode tabref) throws SemanticException {
        ...
        if (tsampleIndex >= 0) {
            ...
            TableSample tabSample = new TableSample(unescapeIdentifier(sampleClause.getChild(0).getText()), unescapeIdentifier(sampleClause.getChild(1).getText()), sampleCols);
            ...
            if (ssampleIndex >= 0) {
                ...
                if (type.getType() == HiveParser.TOK_PERCENT) {
                    ...
                    if (percent < 0 || percent > 100) {
                        throw new SemanticException(generateErrorMessage((ASTNode) numerator, "Sampling percentage should be between 0 and 100"));
                    }
                    int seedNum = conf.getIntVar(ConfVars.HIVESAMPLERANDOMNUM);
                    sample = new SplitSample(percent, seedNum);
                } else {
                    if (type.getType() == HiveParser.TOK_ROWCOUNT) {
   ...
   */
    ...
    private void assertCombineInputFormat(Tree numerator, String message) throws SemanticException {
        String inputFormat = conf.getVar(HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez") ? HiveConf.getVar(conf, HiveConf.ConfVars.HIVETEZINPUTFORMAT) : HiveConf.getVar(conf, HiveConf.ConfVars.HIVEINPUTFORMAT);
        if (!inputFormat.equals(CombineHiveInputFormat.class.getName())) {
            throw new SemanticException(generateErrorMessage((ASTNode) numerator, message + " sampling is not supported in " + inputFormat));
        }
    }
   ...
   */
    ...
    private static boolean isOuterJoinToken(ASTNode node) {
        return (node.getToken().getType() == HiveParser.TOK_LEFTOUTERJOIN) || (node.getToken().getType() == HiveParser.TOK_RIGHTOUTERJOIN) || (node.getToken().getType() == HiveParser.TOK_FULLOUTERJOIN);
    }
            ...
            if (child.getToken().getType() == HiveParser.TOK_TABREF) {
                ...
                if (child.getToken().getType() == HiveParser.TOK_SUBQUERY) {
                    ...
                    if (child.getToken().getType() == HiveParser.TOK_PTBLFUNCTION) {
                        ...
                        if (child.getToken().getType() == HiveParser.TOK_LATERAL_VIEW || child.getToken().getType() == HiveParser.TOK_LATERAL_VIEW_OUTER) {
   ...
   * @throws SemanticException
   */
    @SuppressWarnings(value = { "fallthrough", "nls" })
            ...
            switch(ast.getToken().getType()) {
                ...
                case HiveParser.TOK_DESTINATION:
                    ...
                    if (plannerCtx != null && !queryProperties.hasMultiDestQuery()) {
                        ...
                        if (plannerCtx != null && qbp.getClauseNamesForDest().size() == 2) {
                ...
                case HiveParser.TOK_FROM:
                    ...
                    if (frm.getToken().getType() == HiveParser.TOK_TABREF) {
                        ...
                        if (frm.getToken().getType() == HiveParser.TOK_VIRTUAL_TABLE) {
                            ASTNode newFrom = genValuesTempTable(frm, qb);
                            ast.setChild(0, newFrom);
                            processTable(qb, newFrom);
                        } else {
                            if (frm.getToken().getType() == HiveParser.TOK_SUBQUERY) {
                                processSubQuery(qb, frm);
                            } else {
                                if (frm.getToken().getType() == HiveParser.TOK_LATERAL_VIEW || frm.getToken().getType() == HiveParser.TOK_LATERAL_VIEW_OUTER) {
                                    ...
                                    if (isJoinToken(frm)) {
                                        ...
                                        if (frm.getToken().getType() == HiveParser.TOK_PTBLFUNCTION) {
                                            queryProperties.setHasPTF(true);
                                            processPTF(qb, frm);
                                        }
                                    }
                ...
                case HiveParser.TOK_DISTRIBUTEBY:
                    ...
                    if (qbp.getClusterByForClause(ctx_1.dest) != null) {
                        ...
                        if (qbp.getOrderByForClause(ctx_1.dest) != null) {
                ...
                case HiveParser.TOK_GROUPING_SETS:
                    ...
                    if (ast.getToken().getType() == HiveParser.TOK_ROLLUP_GROUPBY) {
                        ...
                        if (ast.getToken().getType() == HiveParser.TOK_CUBE_GROUPBY) {
                            qbp.getDestCubes().add(ctx_1.dest);
                        } else {
                            if (ast.getToken().getType() == HiveParser.TOK_GROUPING_SETS) {
    ...
    private void handleInsertStatementSpecPhase1(ASTNode ast, QBParseInfo qbp, Phase1Ctx ctx_1) throws SemanticException {
        ...
        if (ast.getType() == HiveParser.TOK_INSERT_INTO && tabColName != null && tabColName.getType() == HiveParser.TOK_TABCOLNAME) {
            ...
            for (Node col : tabColName.getChildren()) {
                assert ((ASTNode) col).getType() == HiveParser.Identifier : "expected token " + HiveParser.Identifier + " found " + ((ASTNode) col).getType();
            ...
            if (targetColNames.size() != targetColumns.size()) {
                throw new SemanticException(generateErrorMessage(tabColName, "Duplicate column name detected in " + fullTableName + " table schema specification"));
            }
            ...
            if (targetTable == null) {
                throw new SemanticException(generateErrorMessage(ast, "Unable to access metadata for table " + fullTableName));
            }
            ...
            if (!targetColumns.isEmpty()) {
                    ...
                    if (tokPartSpec != null) {
                        for (Node n : tokPartSpec.getChildren()) {
                            ...
                            if (tokPartVal != null && tokPartVal.getType() == HiveParser.TOK_PARTVAL && tokPartVal.getChildCount() == 1) {
                                assert tokPartVal.getChild(0).getType() == HiveParser.Identifier : "Expected column name; found tokType=" + tokPartVal.getType();
                ...
                if (!targetColumns.isEmpty()) {
                    throw new SemanticException(generateErrorMessage(tabColName, "\'" + (targetColumns.size() == 1 ? targetColumns.iterator().next() : targetColumns) + "\' in insert schema specification " + (targetColumns.size() == 1 ? "is" : "are") + " not found among regular columns of " + fullTableName + " nor dynamic partition columns."));
                }
            }
        }
    }
                    ...
                    if ("hdfs".equalsIgnoreCase(tablePath.toUri().getScheme())) {
                        if (isPathEncrypted(tablePath)) {
                            if (strongestPath == null) {
                                ...
                                if (comparePathKeyStrength(tablePath, strongestPath) > 0) {
                                    strongestPath = tablePath;
                                }
                            }
                        }
        ...
        switch(condn.getToken().getType()) {
            case HiveParser.TOK_TABLE_OR_COL:
                ...
                if (isPresent(joinTree.getLeftAliases(), tableOrCol)) {
                    ...
                    if (isPresent(joinTree.getRightAliases(), tableOrCol)) {
                    ...
                    if (rightCondAl1.size() != 0) {
                        ...
                        if (leftTree != null && leftTree.getNoOuterJoin()) {
                            ...
                            if (!leftHasRightReference && !rightHasRightReference) {
                                ...
                                if (!leftHasRightReference && rightHasRightReference && rightCondAl1.size() == 1) {
                                    ...
                                    if (leftHasRightReference && !rightHasRightReference && leftCondAl1.size() == 1) {
            ...
            if (leftCondAl2.size() != 0) {
                ...
                if (rightCondAl1.size() != 0) {
                    ...
                    if (type.equals(JoinType.RIGHTOUTER) || type.equals(JoinType.FULLOUTER)) {
                        ...
                        if (type.equals(JoinType.LEFTSEMI)) {
   ...
   */
        ...
        switch(joinCond.getToken().getType()) {
            ...
            case HiveParser.EQUAL:
                ...
                if (((leftCondAl1.size() != 0) && (leftCondAl2.size() != 0)) || ((rightCondAl1.size() != 0) && (rightCondAl2.size() != 0))) {
                    ...
                    applyEqualityPredicateToQBJoinTree(joinTree, type, leftSrc, joinCond, leftCondn, rightCondn, leftCondAl1, leftCondAl2, rightCondAl1, rightCondAl2);
                }
                break;
            default:
                ...
                for (int ci = childrenBegin; ci < joinCond.getChildCount(); ci++) {
                    parseJoinCondPopulateAlias(joinTree, (ASTNode) joinCond.getChild(ci), leftAlias.get(ci - childrenBegin), rightAlias.get(ci - childrenBegin), null, aliasToOpInfo);
                }
    ...
    private Operator genHavingPlan(String dest, QB qb, Operator input, Map<String, Operator> aliasToOpInfo) throws SemanticException {
        ...
        if (!isCBOExecuted() && !qb.getParseInfo().getDestToGroupBy().isEmpty()) {
            ...
            condn = rewriteGroupingFunctionAST(getGroupByForClause(qb.getParseInfo(), destClauseName), condn, !cubeRollupGrpSetPresent);
        }
                        ...
                        for (int i = 1; i < numberOperands; i++) {
                            ...
                            for (int j = 0; j < grpByAstExprs.size(); j++) {
                                ...
                                if (grpByExpr.toStringTree().equals(c.toStringTree())) {
                                    ASTNode childN = (ASTNode) ParseDriver.adaptor.create(HiveParser.IntegralLiteral, String.valueOf(IntMath.mod(-j - 1, grpByAstExprs.size())));
   ...
   */
    ...
    private TableDesc getTableDescFromSerDe(ASTNode child, String cols, String colTypes, boolean defaultCols) throws SemanticException {
        if (child.getType() == HiveParser.TOK_SERDENAME) {
            ...
            try {
                serdeClass = (Class<? extends Deserializer>) Class.forName(serdeName, true, Utilities.getSessionSpecifiedClassLoader());
            } catch (ClassNotFoundException e) {
                throw new SemanticException(e);
            }
            TableDesc tblDesc = PlanUtils.getTableDesc(serdeClass, Integer.toString(Utilities.tabCode), cols, colTypes, defaultCols);
                ...
                for (int numC = 0; numC < numChildRowFormat; numC++) {
                    ...
                    switch(rowChild.getToken().getType()) {
                        ...
                        case HiveParser.TOK_TABLEROWFORMATCOLLITEMS:
                            tblDesc.getProperties().setProperty(serdeConstants.COLLECTION_DELIM, unescapeSQLString(rowChild.getChild(0).getText()));
                            break;
                        case HiveParser.TOK_TABLEROWFORMATMAPKEYS:
                            tblDesc.getProperties().setProperty(serdeConstants.MAPKEY_DELIM, unescapeSQLString(rowChild.getChild(0).getText()));
                            break;
                        case HiveParser.TOK_TABLEROWFORMATLINES:
                            ...
                            if (!lineDelim.equals("\n") && !lineDelim.equals("10")) {
                                throw new SemanticException(generateErrorMessage(rowChild, ErrorMsg.LINES_TERMINATED_BY_NON_NEWLINE.getMsg()));
                            }
                            break;
    ...
    private Operator genScriptPlan(ASTNode trfm, QB qb, Operator input) throws SemanticException {
        ...
        if (trfm.getChildCount() > outputColsNum) {
            ...
            if (outCols.getType() == HiveParser.TOK_ALIASLIST) {
                ...
                if (outCols.getType() == HiveParser.TOK_TABCOLLIST) {
    ...
    private ObjectPair<List<ASTNode>, List<Integer>> getGroupByGroupingSetsForClause(QBParseInfo parseInfo, String dest) throws SemanticException {
        ...
        if (parseInfo.getDestRollups().contains(dest)) {
            ...
            if (parseInfo.getDestCubes().contains(dest)) {
                groupingSets = getGroupingSetsForCube(groupByExprs.size());
            } else {
                if (parseInfo.getDestGroupingSets().contains(dest)) {
   ...
   */
    ...
    static String[] getColAlias(ASTNode selExpr, String defaultName, RowResolver inputRR, boolean includeFuncName, int colNum) {
        ...
        if (selExpr.getChildCount() == 2 || (selExpr.getChildCount() == 3 && selExpr.getChild(2).getType() == HiveParser.TOK_WINDOWSPEC)) {
    ...
    private Operator<?> genSelectPlan(String dest, ASTNode selExprList, QB qb, Operator<?> input, Operator<?> inputForSelectStar, boolean outerLV) throws SemanticException {
        ...
        for (int i = startPosn; i < exprList.getChildCount(); ++i) {
            ...
            if (expr.getType() == HiveParser.TOK_ALLCOLREF) {
                ...
                if (expr.getType() == HiveParser.TOK_TABLE_OR_COL && !hasAsClause && !inputRR.getIsExprResolver() && isRegex(unescapeIdentifier(expr.getChild(0).getText()), conf)) {
                    ...
                    if (expr.getType() == HiveParser.DOT && expr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL && inputRR.hasTableAlias(unescapeIdentifier(expr.getChild(0).getChild(0).getText().toLowerCase())) && !hasAsClause && !inputRR.getIsExprResolver() && isRegex(unescapeIdentifier(expr.getChild(1).getText()), conf)) {
   ...
   */
    ...
    private void addGroupingSetKey(List<ExprNodeDesc> groupByKeys, RowResolver groupByInputRowResolver, RowResolver groupByOutputRowResolver, List<String> outputColumnNames, Map<String, ExprNodeDesc> colExprMap) throws SemanticException {
        ...
        ExprNodeDesc inputExpr = new ExprNodeColumnDesc(TypeInfoFactory.intTypeInfo, groupingSetColumnName, null, false);
   ...
   */
    ...
    private Operator genBucketingSortingDest(String dest, Operator input, QB qb, TableDesc table_desc, Table dest_tab, SortBucketRSCtx ctx) throws SemanticException {
        ...
        if (enforceBucketing) {
            ...
            input = genReduceSinkPlan(input, partnCols, sortCols, order.toString(), nullOrder.toString(), maxReducers, (AcidUtils.isFullAcidTable(dest_tab) ? getAcidType(table_desc.getOutputFileFormatClass(), dest) : AcidUtils.Operation.NOT_ACID));
    ...
    protected Operator genFileSinkPlan(String dest, QB qb, Operator input) throws SemanticException {
            ...
            case QBMetaData.DEST_DFS_FILE:
                {
                    ...
                    loadFileWork.add(new LoadFileDesc(tblDesc, viewDesc, queryTmpdir, dest_path, isDfsDir, cols, colTypes, isMmCtas));
                    if (tblDesc == null) {
                        if (viewDesc != null) {
        ...
        FileSinkDesc fileSinkDesc = createFileSinkDesc(dest, table_desc, dest_part, dest_path, currentTableId, destTableIsAcid, destTableIsTemporary, destTableIsMaterialization, queryTmpdir, rsCtx, dpCtx, lbCtx, fsRS, canBeMerged, txnId, isMmCtas);
        if (isMmCtas) {
        ...
        if (dest_tab != null && conf.getBoolVar(ConfVars.HIVESTATSAUTOGATHER) && conf.getBoolVar(ConfVars.HIVESTATSCOLAUTOGATHER) && ColumnStatsAutoGatherContext.canRunAutogatherStats(fso)) {
            if (dest_type.intValue() == QBMetaData.DEST_TABLE) {
                ...
                if (dest_type.intValue() == QBMetaData.DEST_PARTITION) {
    ...
    private ColsAndTypes deriveFileSinkColTypes(RowResolver inputRR, List<FieldSchema> field_schemas) throws SemanticException {
    ...
    private FileSinkDesc createFileSinkDesc(String dest, TableDesc table_desc, Partition dest_part, Path dest_path, int currentTableId, boolean destTableIsAcid, boolean destTableIsTemporary, boolean destTableIsMaterialization, Path queryTmpdir, SortBucketRSCtx rsCtx, DynamicPartitionCtx dpCtx, ListBucketingCtx lbCtx, RowSchema fsRS, boolean canBeMerged, Long mmWriteId, boolean isMmCtas) throws SemanticException {
        FileSinkDesc fileSinkDesc = new FileSinkDesc(queryTmpdir, table_desc, conf.getBoolVar(HiveConf.ConfVars.COMPRESSRESULT), currentTableId, rsCtx.isMultiFileSpray(), canBeMerged, rsCtx.getNumFiles(), rsCtx.getTotalFiles(), rsCtx.getPartnCols(), dpCtx, dest_path, mmWriteId, isMmCtas);
        boolean isHiveServerQuery = SessionState.get().isHiveServerQuery();
        fileSinkDesc.setHiveServerQuery(isHiveServerQuery);
        // If this is an insert, update, or delete on an ACID table then mark that so the
        // FileSinkOperator knows how to properly write to it.
        boolean isDestInsertOnly = (dest_part != null && dest_part.getTable() != null && MetaStoreUtils.isInsertOnlyTable(dest_part.getTable().getParameters())) || (table_desc != null && MetaStoreUtils.isInsertOnlyTable(table_desc.getProperties()));
        if (isDestInsertOnly) {
    ...
    private void handleLineage(LoadTableDesc ltd, Operator output) throws SemanticException {
        if (ltd != null && SessionState.get() != null) {
            ...
            } catch (MetaExceptionHiveException |  e) {
                throw new SemanticException(e);
            }
    ...
    private void checkImmutableTable(QB qb, Table dest_tab, Path dest_path, boolean isPart) throws SemanticException {
        ...
        try {
            ...
            LOG.warn("Error while trying to determine if immutable table " + (isPart ? "partition " : "") + "has any data : " + dest_tab.getTableName() + " : " + dest_path);
    ...
    private void genAutoColumnStatsGatheringPipeline(QB qb, TableDesc table_desc, Map<String, String> partSpec, Operator curr, boolean isInsertInto) throws SemanticException {
        ...
        LOG.info("Generate an operator pipeline to autogather column stats for table " + tableName + " in query " + ctx.getCmd());
    ...
    // that isn't true.
    private void checkAcidConstraints(QB qb, TableDesc tableDesc, Table table, AcidUtils.Operation acidOp) throws SemanticException {
            ...
            for (int i = 0; i < ccount; ++i) {
                ...
                if (cl.getType() == HiveParser.TOK_TABSORTCOLNAMEASC) {
                    ...
                    if (cl.getType() == HiveParser.TOK_NULLS_FIRST) {
                        ...
                        if (cl.getType() == HiveParser.TOK_NULLS_LAST) {
                            nullOrder.append("z");
                        } else {
                    ...
                    if (cl.getType() == HiveParser.TOK_TABSORTCOLNAMEDESC) {
                        order.append("-");
                        cl = (ASTNode) cl.getChild(0);
                        if (cl.getType() == HiveParser.TOK_NULLS_FIRST) {
                            nullOrder.append("a");
   ...
   */
    private QBJoinTree genSQJoinTree(QB qb, ISubQueryJoinInfo subQuery, Operator joiningOp, Map<String, Operator> aliasToOpInfo) throws SemanticException {
        QBJoinTree joinTree = new QBJoinTree();
    ...
    private static boolean isValidJoinSide(ASTNode right) {
        return (right.getToken().getType() == HiveParser.TOK_TABREF) || (right.getToken().getType() == HiveParser.TOK_SUBQUERY) || (right.getToken().getType() == HiveParser.TOK_PTBLFUNCTION);
    }
    ...
    private void mergeJoinTree(QB qb) {
        ...
        for (int i = trees.size() - 1; i >= 0; i--) {
            ...
            for (int j = i - 1; j >= 0 && continueScanning; j--) {
                ...
                if ((!node.getNoOuterJoin() && node.getPostJoinFilters().size() != 0) || (!target.getNoOuterJoin() && target.getPostJoinFilters().size() != 0)) {
                    break;
                }
   ...
   */
    ...
    private void setupStats(TableScanDesc tsDesc, QBParseInfo qbp, Table tab, String alias, RowResolver rwsch) throws SemanticException {
        if ((!qbp.isAnalyzeCommand() && qbp.getAnalyzeRewrite() == null) || (qbp.getAnalyzeRewrite() != null && !HiveConf.getVar(conf, HiveConf.ConfVars.HIVE_EXECUTION_ENGINE).equals("tez"))) {
   ...
   */
    ...
    boolean genResolvedParseTree(ASTNode ast, PlannerContext plannerCtx) throws SemanticException {
        ...
        if (ast.getToken().getType() == HiveParser.TOK_CREATEVIEW || ast.getToken().getType() == HiveParser.TOK_CREATE_MATERIALIZED_VIEW || (ast.getToken().getType() == HiveParser.TOK_ALTERVIEW && ast.getChild(1).getType() == HiveParser.TOK_QUERY)) {
        ...
        switch(ast.getToken().getType()) {
            case HiveParser.TOK_SET_AUTOCOMMIT:
                ...
                if (ast.getChild(0).getType() == HiveParser.TOK_TRUE) {
                    ...
                    if (ast.getChild(0).getType() == HiveParser.TOK_FALSE) {
            ...
            case HiveParser.TOK_ROLLBACK:
                if (!(conf.getBoolVar(ConfVars.HIVE_IN_TEST) || conf.getBoolVar(ConfVars.HIVE_IN_TEZ_TEST))) {
                    throw new IllegalStateException(SemanticAnalyzerFactory.getOperation(ast.getToken().getType()) + " is not supported yet.");
                }
        ...
        for (int num = 1; num < numCh; num++) {
            ...
            switch(child.getToken().getType()) {
                ...
                case HiveParser.TOK_TABCOLLIST:
                    cols = getColumns(child, true, primaryKeys, foreignKeys, uniqueConstraints, notNullConstraints);
                    break;
        ...
        while (!stack.isEmpty()) {
            ...
            for (int child_pos = 0; child_pos < child_count; ++child_pos) {
                ...
                if (type == HiveParser.TOK_SELECT) {
                    ...
                    if (type == HiveParser.TOK_GROUPBY) {
                        groupbyNode = node;
                    } else {
                        if (type == HiveParser.TOK_ORDERBY) {
                            orderbyNode = node;
                        }
                    }
                }
   ...
   */
    ...
    private PartitioningSpec processPTFPartitionSpec(ASTNode pSpecNode) {
        ...
        if (type == HiveParser.TOK_DISTRIBUTEBY || type == HiveParser.TOK_CLUSTERBY) {
            ...
            if (type == HiveParser.TOK_SORTBY || type == HiveParser.TOK_ORDERBY) {
   ...
   */
    ...
    private void addAlternateGByKeyMappings(ASTNode gByExpr, ColumnInfo colInfo, Operator<? extends OperatorDesc> reduceSinkOp, RowResolver gByRR) {
        if (gByExpr.getType() == HiveParser.DOT && gByExpr.getChild(0).getType() == HiveParser.TOK_TABLE_OR_COL) {
            ...
            if (gByExpr.getType() == HiveParser.TOK_TABLE_OR_COL) {
    ...
    private WriteEntity.WriteType determineWriteType(LoadTableDesc ltd, boolean isNonNativeTable, String dest) {
        if (isNonNativeTable) {
            ...
            return (ltd.getReplace() ? WriteEntity.WriteType.INSERT_OVERWRITE : getWriteType(dest));
        }
    }
    ...
    private void copyInfoToQueryProperties(QueryProperties queryProperties) {
        if (qb != null) {
            ...
            queryProperties.setHasOuterOrderBy(!qb.getParseInfo().getIsSubQ() && !qb.getParseInfo().getDestToOrderBy().isEmpty());
