...
public class InsertBase extends AbstractBaseTest {
    ...
    public void Insert_Dates() {
        ...
        insert.execute();
        Tuple result = query().from(dateTest).singleResult(dateTest.dateTest.year(), dateTest.dateTest.month(), dateTest.dateTest.dayOfMonth(), dateTimeProperty);
    ...
    public void Complex1() {
        ...
        insert.select(new SQLSubQuery().from(survey).innerJoin(emp1).on(survey.id.eq(emp1.id)).innerJoin(emp2).on(emp1.superiorId.eq(emp2.superiorId), emp1.firstname.eq(emp2.firstname)).list(survey.id, emp2.firstname));
        insert.execute();
    }
    ...
    @Test
    @ExcludeIn(value = { CUBRID, SQLSERVER })
    ...
    @Test
    @ExcludeIn(value = FIREBIRD)
    public void Insert_With_SubQuery() {
        int count = (int) query().from(survey).count();
        assertEquals(count, insert(survey).columns(survey.id, survey.name).select(sq().from(survey2).list(survey2.id.add(20), survey2.name)).execute());
    }

    @Test
    @ExcludeIn(value = { HSQLDB, CUBRID, DERBY, FIREBIRD })
    public void Insert_With_SubQuery2() {
        assertEquals(1, insert(survey).set(survey.name, sq().where(sq().from(survey2).where(survey2.name.eq("MyModule")).notExists()).unique(Expressions.constant("MyModule"))).execute());
        assertEquals(1l, query().from(survey).where(survey.name.eq("MyModule")).count());
    }

    @Test
    @ExcludeIn(value = { HSQLDB, CUBRID, DERBY })
    public void Insert_With_SubQuery3() {
        assertEquals(1, insert(survey).columns(survey.name).select(sq().where(sq().from(survey2).where(survey2.name.eq("MyModule2")).notExists()).unique(Expressions.constant("MyModule2"))).execute());
    ...
    public void Insert_With_SubQuery_Params() {
        ...
        sq.set(param, 20);
        int count = (int) query().from(survey).count();
        assertEquals(count, insert(survey).columns(survey.id, survey.name).select(sq.list(survey2.id.add(param), survey2.name)).execute());
    }
    ...
    public void Insert_With_SubQuery_Without_Columns() {
        int count = (int) query().from(survey).count();
        assertEquals(count, insert(survey).select(sq().from(survey2).list(survey2.id.add(10), survey2.name, survey2.name2)).execute());
    }
    ...
    public void InsertBatch_with_Subquery() {
        SQLInsertClause insert = insert(survey).columns(survey.id, survey.name).select(sq().from(survey2).list(survey2.id.add(20), survey2.name)).addBatch();
        insert(survey).columns(survey.id, survey.name).select(sq().from(survey2).list(survey2.id.add(40), survey2.name)).addBatch();
        insert.execute();
    }
